...
export const BuyurtmaTable = ({ status }: { status: IStatus }) => {
  ...
  const table: IBuyurtma[] = buyurtmalar.filter((b) => b.status === status);
  ...
  const handleIncrement = (id: number | string) => {
    const buyurtmaIndex = buyurtmalar.findIndex((b) => b.id === id);
  ...
  const confirmDelete = () => {
    if (itemToDelete !== null) {
      const buyurtmaIndex = buyurtmalar.findIndex((b) => b.id === itemToDelete);
  ...
  return (
    <div className="ps-9 pe-11 py-7 overflow-y-auto">
      {table.map((t, index) => {
        const user = mijozlar.find((m) => m.id === t.userId);
        ...
        const price = t.mahsulotlar.reduce((total, mahsulot) => {
          const res = mahsulotlar.find((m) => m.id === mahsulot.mahsulotId);

          return total + (res ? mahsulot.count * res.narx : 0);
        }, 0);

        const filial = filiallar.find((f) => f.id === t.filialId);

        const hodim = hodimlar.find((h) => h.id === t.hodimId);
        ...
        return (
          ...
          >
            <Grid container>
              <Grid item xs={6} lg={3} sx={{ padding: "20px 35px" }}>
                <div className="flex">
                  <div
                ...
                <div className="flex justify-between">
                  <div className="">
                    <div className="flex gap-2">
                      <LuClipboard /> <p className="text-lg">{price} UZS</p>
                    </div>
                    <div className="flex gap-2">
                      <LuTruck />
              ...
              >
                <div className="mb-4">
                  ...
                  <p className="text-xl">
                    {hodim?.lastName} {hodim?.firstName.charAt(0)}
                  </p>
                </div>
                <div>
                  <p className="text-gray-600">Filial:</p>
                  <p className="text-xl font-semibold">{filial?.nameUz}</p>
                </div>
                <div
                  style={{
                    position: "absolute",
                ...
                >
                  ...
                  >
                    <IconButton
                      disabled={t.status === "yopilgan" ? true : false}
                      onClick={() => {
                        handleIncrement(t.id);
                      }}
                    >
                      <IoMdCheckmark />
                    </IconButton>
